            ...
            for region, region_eq in sta_s_p_df.groupby("region"):
                ...
                for _, eq in region_eq.iterrows():
                    ...
                    for i in range(n_boot):
                        ## another method median
                        sample = vpvs_samples[i]
                        vpvs_mode_array[i] = np.mean(sample)

                        ### method 1
                        # sample = vpvs_samples[i]

                        # # If sample is too small or constant → fallback to median
                        # if len(sample) < 3 or np.std(sample) < 1e-6:
                        #     vpvs_mode_array[i] = np.median(sample)
                        #     continue

                        # # KDE for this bootstrap sample
                        # kde_bs = KernelDensity(kernel="gaussian", bandwidth=0.02).fit(sample.reshape(-1, 1))

                        # # Evaluate KDE on a fine grid within sample range
                        # x_vals_bs = np.linspace(sample.min(), sample.max(), 300).reshape(-1, 1)
                        # log_dens_bs = kde_bs.score_samples(x_vals_bs)

                        # # Mode of KDE
                        # vpvs_mode_array[i] = x_vals_bs[np.argmax(log_dens_bs)][0]


                        ####################################### OTHER METHOD: HISTOGRAM MODE #######################################
