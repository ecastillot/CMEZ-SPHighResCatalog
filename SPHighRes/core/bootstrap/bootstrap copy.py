...
def eq_depths(
    ...
    for radius, station_paths in radii_dict.items():
        ...
        for station, path in station_paths:
            ...
            # --- Keep only values around the trend ---
            q25, q75 = np.percentile(vpvs_data, [25, 75])
            iqr = q75 - q25
            lower, upper = q25 - trend_sigma*iqr, q75 + trend_sigma*iqr
            vpvs_filtered = vpvs_data[(vpvs_data >= lower) & (vpvs_data <= upper)]
            if len(vpvs_filtered) == 0:
                print(f"All VP/VS values are outliers for event {eq['ev_id']}, skipping.")
                continue

            mean_v = np.mean(vpvs_filtered) 
            std_v = np.std(vpvs_filtered) 
            lower = mean_v - trend_sigma * std_v 
            upper = mean_v + trend_sigma * std_v
            vpvs_filtered = vpvs_filtered[(vpvs_filtered >= lower) & (vpvs_filtered <= upper)]
            ...
            for region, region_eq in sta_s_p_df.groupby("region"):
                ...
                for _, eq in region_eq.iterrows():
                    ...
                    alpha_array = ts_tp / (vpvs_mode_array - 1)
